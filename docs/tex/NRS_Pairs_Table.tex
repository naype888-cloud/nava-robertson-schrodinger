...
gives the light cone (\lean{cono\_de\_luz}, \lean{cono\_de\_luz\_cubo}, \lean{D37f}); in
...
of each mode (\lean{Td\_mulVec\_modoSeno}, \lean{D38}). Time and transport are tied by the
...
time, and $\psi^*$ moves at exactly that speed (\lean{abs\_velocidad\_le},
\lean{velocidad\_psiStar}); on the cube $\Psi^*$ does so on all three axes at once
(\lean{velocidades\_PsiStar3D}). So rows 26 and 27 are the dynamics that $T_d$ generates on
