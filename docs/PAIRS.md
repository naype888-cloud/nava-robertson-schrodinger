...
mode (`Td_mulVec_modoSeno`, `D38`). The Heisenberg equation `d/dt (U† P_d U) = U† K_d U` ties
time to transport (`heisenberg`): the tension is velocity, and `ψ*` moves at exactly one site
per unit of time (`velocidad_psiStar`), on all three axes of the cube at once
(`velocidades_PsiStar3D`). Rows 26 and 27 are the dynamics `T_d` generates on the axes where
rows 1–25 live.

Rows 1–3 are the three axes of the cube of `D37`; any three rows among 1–25 whose pairs commute
with each other can occupy the three axes, and `cuanto_volumetrico_pares` applies to them.
