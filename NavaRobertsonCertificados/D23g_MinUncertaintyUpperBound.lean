...
/-!
# D23g — Minimum-uncertainty states of `H₄` carry tension at most `1/φ`

`D23f` exhibits a minimum-uncertainty state of `H₄` with tension `1/φ = (√5 − 1)/2`. No
minimum-uncertainty state goes higher, so `1/φ` is the maximal tension compatible with
saturation at `d = 4`: `3(√5 − 1)/4 ≈ 92.7 %` of `2/3`.

Proof. Saturation gives `centered T₄ = c · centered P₄` (`D23`); row by row, the link currents
`Iⱼ = −Im(ψ̄ⱼ ψⱼ₊₁)` satisfy `Iⱼ = s cⱼ(n)` with `s = ρ₄ Im c` and `cⱼ` quadratic in the densities
`nⱼ = |ψⱼ|²`. Cauchy–Schwarz gives `Iⱼ² ≤ nⱼ nⱼ₊₁`, and `⟨K₄⟩ = (2/ρ₄) s Var P`. The three link
bounds force `4 s² (Var P)² ≤ 1` (`DensityBound.density_bound`), so `⟨K₄⟩ ≤ 1/φ`. Assuming
`⟨P⟩ ≥ 0` by reflection, the sign of `H = c₁² − n₁ n₃ (1 − ⟨P⟩)²` selects the tightest link, and
each case is an exact rational Positivstellensatz certificate (`CotaCasoA`, `CotaCasoB`: 207 and
229 weighted squares, checked by `ring`).

## Main results

- `MinUncertaintyBoundFour.tension_le_of_saturated` : every minimum-uncertainty state of `H₄` has
  tension at most `1/φ`.
-/

@[expose] public section

/-! ## The density form of the bound -/

noncomputable section

namespace DensityBound

/-- The total weight `S = Σ nⱼ`. -/
...
/-- The link coefficients `cⱼ S`: the current on link `j` is `s cⱼ`. -/
...
/-- The region polynomial: `H ≥ 0`, the middle link is tightest; `H ≤ 0`, the right one. -/
def H (n0 n1 n2 n3 : ℝ) : ℝ :=
  c1 n0 n1 n2 n3 ^ 2 - n1 * n3 * (S n0 n1 n2 n3 - mP n0 n1 n2 n3) ^ 2

variable {n0 n1 n2 n3 : ℝ}

/-- Case A: `m ≥ 0`, `H ≥ 0` gives `4 V² n₁ n₂ ≤ c₁² S²`. -/
theorem caseA (h0 : 0 ≤ n0) (h1 : 0 ≤ n1) (h2 : 0 ≤ n2) (h3 : 0 ≤ n3)
    (hm : 0 ≤ mP n0 n1 n2 n3) (hH : 0 ≤ H n0 n1 n2 n3) :
    4 * V n0 n1 n2 n3 ^ 2 * n1 * n2 ≤ c1 n0 n1 n2 n3 ^ 2 * S n0 n1 n2 n3 ^ 2 := by
  have := CertificateH4.bound_caseA n0 n1 n2 n3 (mP n0 n1 n2 n3) (H n0 n1 n2 n3) h0 h1 h2 h3
    (by unfold mP; ring) (by unfold H c1 mP S; ring) hm hH
  rw [← sub_nonneg]
  convert this using 2
  unfold c1 V mP S; ring

/-- Case B: `m ≥ 0`, `H ≤ 0` gives `4 V² n₂ ≤ (S − m)² n₃ S²`. -/
theorem caseB (h0 : 0 ≤ n0) (h1 : 0 ≤ n1) (h2 : 0 ≤ n2) (h3 : 0 ≤ n3)
    (hm : 0 ≤ mP n0 n1 n2 n3) (hH : H n0 n1 n2 n3 ≤ 0) :
    4 * V n0 n1 n2 n3 ^ 2 * n2 ≤
      (S n0 n1 n2 n3 - mP n0 n1 n2 n3) ^ 2 * n3 * S n0 n1 n2 n3 ^ 2 := by
  have := CertificateH4.bound_caseB n0 n1 n2 n3 (mP n0 n1 n2 n3) (-H n0 n1 n2 n3) h0 h1 h2 h3
...
/-- The bound for `⟨P⟩ ≥ 0`. -/
theorem mP_nonneg (h0 : 0 ≤ n0) (h1 : 0 ≤ n1) (h2 : 0 ≤ n2) (h3 : 0 ≤ n3)
  ...
  · -- right link
    have hB := caseB h0 h1 h2 h3 hm hH
    rw [hS] at hB
  ...
  · -- middle link
    have hA := caseA h0 h1 h2 h3 hm hH.le
    rw [hS] at hA
...
/-- **Density form.** If `(s cⱼ)² ≤ nⱼ nⱼ₊₁` on every link, then `4 s² (Var P)² ≤ 1`. -/
theorem density_bound (h0 : 0 ≤ n0) (h1 : 0 ≤ n1) (h2 : 0 ≤ n2) (h3 : 0 ≤ n3)
  ...
  · exact mP_nonneg h0 h1 h2 h3 hS hm s k1 k2
  · -- mirror n ↦ (n₃, n₂, n₁, n₀)
    have r := mP_nonneg h3 h2 h1 h0 (by unfold S at *; linarith)
...
namespace MinUncertaintyBoundFour

open TransportPosition NRSInequality SpectralExtremal EigenvectorSaturation
open MinUncertaintyFour DensityBound Complex

/-- The tension of `ψ ∈ H₄` is `(4/(3ρ₄)) (I₀ + I₁ + I₂)`, `Iⱼ = −Im(ψ̄ⱼ ψⱼ₊₁)`. -/
theorem tension_currents (ψ : Hd 4) :
    NearMaxTension.tension 4 ψ =
      4 / (3 * rho 4) * (((ψ 0).im * (ψ 1).re - (ψ 0).re * (ψ 1).im) +
        ((ψ 1).im * (ψ 2).re - (ψ 1).re * (ψ 2).im) +
        ((ψ 2).im * (ψ 3).re - (ψ 2).re * (ψ 3).im)) := by
  have hr := rho_four_pos
  rw [NearMaxTension.tension, re_inner_KdOp, TdOp_four_apply, PdOp_four_apply]
...
theorem normSq_sum (ψ : Hd 4) :
    ‖ψ‖ ^ 2 = normSq (ψ 0) + normSq (ψ 1) + normSq (ψ 2) + normSq (ψ 3) := by
  rw [EuclideanSpace.norm_sq_eq, Fin.sum_univ_four]
  simp [Complex.normSq_eq_norm_sq]

theorem mean_P_coord (ψ : Hd 4) :
    mean (PdOp 4) ψ = mP (normSq (ψ 0)) (normSq (ψ 1)) (normSq (ψ 2)) (normSq (ψ 3)) := by
  rw [mean, PdOp_four_apply]
  simp only [PiLp.inner_apply, Fin.sum_univ_four, RCLike.inner_apply, Matrix.cons_val_zero,
    Matrix.cons_val_one, Matrix.cons_val_two, Matrix.cons_val_three, Matrix.head_cons,
...
theorem tension_le_of_saturated (ψ : Hd 4) (hψ : ‖ψ‖ = 1)
    (hsat : gramDefectAt (TdOp 4) (PdOp 4) ψ = 0) :
    NearMaxTension.tension 4 ψ ≤ (√5 - 1) / 2 := by
  have hr := rho_four_pos
  ...
  have hpos : 0 < (√5 - 1) / 2 := by
    ...
    linarith
  rcases (gramDefectAt_eq_zero_iff _ _ _).mp hsat with hvar | ⟨c, hc⟩
  · -- `Var P = 0`: the tension vanishes
    have h := EigenvectorSaturation.variance_mul_variance_ge 4 ψ
    rw [hvar, mul_zero] at h
    have : (inner ℂ ψ (KdOp 4 ψ)).re = 0 := by nlinarith [sq_nonneg (inner ℂ ψ (KdOp 4 ψ)).re]
    rw [NearMaxTension.tension, this]; exact hpos.le
  · -- coordinates
    set t := mean (TdOp 4) ψ
    set p := mean (PdOp 4) ψ
    set h := 1 / rho 4 with hh
    have hrh : rho 4 * h = 1 := by rw [hh]; field_simp
    have comp : ∀ j : Fin 4, (centered (TdOp 4) ψ) j = c * (centered (PdOp 4) ψ) j := by
      intro j; rw [hc]; rfl
    have e0 := comp 0; have e1 := comp 1; have e3 := comp 3
    simp only [centered, TdOp_four_apply, PdOp_four_apply, PiLp.sub_apply, PiLp.smul_apply,
    ...
    have hS : S (normSq (ψ 0)) (normSq (ψ 1)) (normSq (ψ 2)) (normSq (ψ 3)) = 1 := by
      have := normSq_sum ψ; rw [hψ] at this; unfold S; linarith
    have hp : p = mP (normSq (ψ 0)) (normSq (ψ 1)) (normSq (ψ 2)) (normSq (ψ 3)) := mean_P_coord ψ
    ...
    have k0 : (sc * c0 N0 N1 N2 N3) ^ 2 ≤ N0 * N1 := by rw [← J0]; exact current_cs _ _
    have k1 : (sc * c1 N0 N1 N2 N3) ^ 2 ≤ N1 * N2 := by rw [← J1]; exact current_cs _ _
    have k2 : (sc * c2 N0 N1 N2 N3) ^ 2 ≤ N2 * N3 := by rw [← J2]; exact current_cs _ _
    ...
    have key := density_bound hN0 hN1 hN2 hN3 hS sc k0 k1 k2
    -- tension = (2/ρ₄) s V
    have hsum : c0 N0 N1 N2 N3 + c1 N0 N1 N2 N3 + c2 N0 N1 N2 N3 = 3 / 2 * V N0 N1 N2 N3 := by
      unfold c0 c1 c2 V mP S; ring
    have ht : NearMaxTension.tension 4 ψ = 2 / rho 4 * (sc * V N0 N1 N2 N3) := by
      rw [tension_currents]
